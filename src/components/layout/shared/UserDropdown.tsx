...
const UserDropdown = () => {
  ...
  const handleUserLogout = async () => {
    try {
      ...
      // signOut removed; use custom logout logic
      removeToken();
              ...
              <ClickAwayListener onClickAway={e => handleDropdownClose(e as MouseEvent | TouchEvent)}>
                <MenuList>
                  <div className='flex items-center plb-2 pli-6 gap-2' tabIndex={-1}>
                    <Avatar alt={JSON.parse(localStorage.getItem('user') || '')?.name || ''} src={JSON.parse(localStorage.getItem('user') || '')?.image || ''} />
                    <div className='flex items-start flex-col'>
                      <Typography className='font-medium' color='text.primary'>
                        {JSON.parse(localStorage.getItem('user') || '')?.name || ''}
                      </Typography>
                      <Typography variant='caption'>{JSON.parse(localStorage.getItem('user') || '')?.email || ''}</Typography>
                    </div>
                  </div>
